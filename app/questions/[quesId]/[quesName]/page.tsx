import { TracingBeam } from '@/app/components/aceternity/tracing-beam'
...
const Page = async ({params}: {params: {quesId: string, quesName: string}}) => {
    const [question, answers, upvotes, downvotes, comments] = await Promise.all([
        ...
        databases.listDocuments(db, answerCollection, [
            Query.orderDesc("$createdAt"), Query.equal("questionId", "params.quesId")
        ]),
        databases.listDocuments(db, voteCollection, [
            Query.equal("typeId", "params.quesId"), Query.equal("type", "question"), Query.equal("voteStatus", "upvoted"), Query.limit(1)
        ]),
        databases.listDocuments(db, voteCollection, [
            Query.equal("typeId", "params.quesId"), Query.equal("type", "question"), Query.equal("voteStatus", "downvoted"), Query.limit(1)
        ]),
        databases.listDocuments(db, commentCollection, [
            Query.orderDesc("$createdAt"), Query.equal("type", "question"), Query.equal("typeId", "params.quesId")
        ]),
    ])
