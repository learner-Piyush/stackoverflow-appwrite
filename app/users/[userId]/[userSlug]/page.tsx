...
const Page = async ({params}: {params: {userId: string, userSlug: string}}) => {
    const [user, questions, answers] = await Promise.all([
        ...
        databases.listDocuments(db, questionCollection, [Query.equal("authorId", params.userId), Query.limit(1)]),
        databases.listDocuments(db, answerCollection, [Query.equal("authorId", params.userId), Query.limit(1)]),
    ])
  return (
