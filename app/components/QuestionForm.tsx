...
const QuestionForm = ({ question }: { question?: (Models.Document & { title?: string; content?: string; tags?: string[]; attachmentId?: string }) }) => {

  const { user } = useAuthStore();
  const [tag, setTag] = React.useState("");
  const router = useRouter();

const [formData, setFormData] = React.useState<{ title: string; content: string; authorId?: string; tags: Set<string>; attachment: File | null } >({
  ...
  const loadConfetti = (timeInMS = 3000) => {
    ...
    const frame = () => {
      ...
      // canvas-confetti wrapper types don't support particleCount directly
      (Confetti as unknown as { fire?: (opts: Record<string, unknown>) => void }).fire?.({

        angle: 60,
        spread: 55,
        startVelocity: 60,
        origin: { x: 0, y: 0.5 },
        // wrapper typing differs from canvas-confetti options; particleCount is tolerated at runtime
        particleCount: 2,
        colors: colors,
      });
      (Confetti as unknown as { fire?: (opts: Record<string, unknown>) => void }).fire?.({
