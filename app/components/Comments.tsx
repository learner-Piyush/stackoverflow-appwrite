...
type CommentUI = Models.Document & {
  content: string;
  authorId: string;
  author: { name: string };
};
...
const Comments = ({
  ...
  const [comments, setComments] = React.useState(_comments as unknown as Models.DocumentList<CommentUI>);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setComments((prev) => ({
        total: prev.total + 1,
        documents: [
          { ...(response as Models.Document), author: user } as unknown as CommentUI,
          ...prev.documents,
        ],
      }));
    } catch (error) {
